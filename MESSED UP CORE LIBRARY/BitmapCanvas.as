package 
{
	...
	public class  BitmapCanvas extends Sprite
	{
		...
		public function print(stringID:*, xcod:Number, ycod:Number, placeMid:Boolean = false):void
		{
			var eNum:int
			...
			if (placeMid)
			{
				bitmapDat.copyPixels(bmd, new Rectangle(0, 0, bRes.x, bRes.y), new Point(xcod - bmd.width/2, ycod - bmd.height/2), null, new Point(0, 0), true);
			}else
			{
				bitmapDat.copyPixels(bmd, new Rectangle(0, 0, bRes.x, bRes.y), new Point(xcod, ycod), null, new Point(0, 0), true);
			}
		}
